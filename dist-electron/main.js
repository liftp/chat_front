...
const db = new Datastore$1({
  filename: "./chatDB.db",
  autoload: true
  // inMemoryOnly: false, // 是否仅存在内存中
  // timestampData: false, // 是属于creaedAt标记所有文档的插入和最后更新时间updatedAt
});
const dbFreinds = new Datastore$1({
  filename: "./friendDB.db",
  autoload: true
  // inMemoryOnly: false, // 是否仅存在内存中
  // timestampData: false, // 是属于creaedAt标记所有文档的插入和最后更新时间updatedAt
});
const readRecord = (start, end, search) => {
  return new Promise((resolve, reject) => {
    db.find({ saveType: "1", ...search }).sort({ createdAt: -1 }).skip(start).limit(end - start).exec((err, docs) => {
      if (err != null) {
        reject(err);
      }
      resolve(docs);
...
const findFriend = (name, selfId) => {
  return new Promise((resolve, reject) => {
    dbFreinds.find({ friendName: new RegExp(name), selfId }).exec((err, docs) => {
      if (err != null) {
        reject(err);
      }
      resolve(docs);
...
const __dirnameNew = path$3.dirname(__filenameNew);
console.log(__dirnameNew);
process$1.env["ELECTRON_DISABLE_SECURITY_WARNINGS"] = "true";
function createWindow() {
  const mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: path$3.join(__dirnameNew, "../src/preload/index.mjs"),
      sandbox: false
